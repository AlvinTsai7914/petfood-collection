...
- v4 複測(2026-08-24~26):全端點重測 + `ingredient` / `excludeIngredient` 篩選探測 + 成分陣列斷裂實例取樣;前端已部署兩個線上環境可供對照(見「線上示範環境」)
**對照基準**:`docs/api-alignment-260429.md` v1.4(2026-05-06)、`docs/api-260429.txt`(後端 v2 spec)
**目的**:列出 live API 與既有對齊文件的所有差異,供後端會議逐項收斂

---

## 線上示範環境(v4 新增,2026-08-25 部署)

| 環境 | 網址 | 用途 |
|------|------|------|
| Mock 版 | https://petfood-collection.vercel.app | 前端期望的完整樣子(30 筆 mock,欄位齊全) — 看「補完後長怎樣」 |
| Live 版 | https://petfood-collection-live.vercel.app | 直接代理 live 後端(2087 筆真實資料) — 當場看 #0 的空列表、#13 的詳情頁 404 |

## 現況速覽(2026-08-24~26 複測)

| # | 議題 | v3 狀態 | v4 最新複測結果 |
|---|------|---------|-------------|
| 0 | API 間歇性 500 | 🔴 | 🔴 **仍在發生**:8/24、8/25、8/26 三天皆重現(首次請求 500、重試即過),filters / products / 帶篩選查詢都中過 |
| 13 | `GET /api/products/{id}` 不存在 | 🔴 | 🔴 未解:仍 404。前端已做「列表點擊帶資料」過渡(站內導覽可用),但**直接進入 / 重新整理 / 分享連結 / SEO 收錄仍全死**,維持 P0 |
| 3 | ingredients 拆陣列 | 🔴 | 🟡 **大幅降級**:實測 `ingredient` / `excludeIngredient` 篩選已可用(見 #3 v4 更新),字典乾淨;剩顯示層問題 — 補一個 `ingredientsText` 欄位即結案 |
| 1 | 濕食佔比 6.5% | 🟠 | 🟠 未變,待產品決策 |
| 2 | age enum 對不上 | 🔴 | 🔴 未解:仍 `puppy/adult/senior`,無 `kitten`/`all` |
| 4 | variants[] 未規格化 | 🔴 | 🔴 未解:欄位仍無文件 |
| 5-12 | P1 群 | 🟡 | 🟡 #8 fiberPct 仍 0% 暴露(v4 複測確認欄位不存在);其餘未變 |

**前端側現況(2026-08-26)**:詳情頁(F16)已完成並上線;兩個示範環境已部署(見上表);過渡方案已上版 — (a) 列表點卡片把該筆資料帶進詳情頁,live 站內導覽不再 404;(b) `ingredientsText` 缺失時前端把 `ingredients[]` 用「、」黏回近似原文顯示。**真正擋切 live 的只剩 #0 / #13 兩項**。
...
> **開會用的一句話(v4 修訂)**:必須改 code 的剩 3 項 — #0(查根因)、#13(單筆端點,查詢邏輯同列表只差 `WHERE id = ?`)、#8(fiberPct 暴露,資料已在手上);#3 降級為「順手加一個 `ingredientsText` 欄位」(半小時等級,不擋上線)。其餘是文件補登、產品決策與爬蟲 backlog。
...
- **現況**:故障在端點間漂移、無固定模式 —
  ...
  - 8/24(v4):`/api/filters` 首次 500、重試 2 次皆 200
  - 8/25(v4):`?limit=1` 帶篩選查詢首次 500、重試即過
  - 8/26(v4):`?ingredient=雞` 首次 500、重試即過 — **三天連續重現,模式一致:首次請求易 500、重試即恢復**,更支持 DB 連線池/冷啟動方向
...
- **v4 補充 — 前端過渡方案已上版,但只救了站內導覽**:列表點卡片會把該筆資料帶進詳情頁(不打 API),live 站內瀏覽已可用;然而**直接進入 / 重新整理 / 分享連結 / SEO 收錄**四種情境瀏覽器手上只有 id,仍需此端點,維持 P0。對「產品資料庫」網站而言分享連結與 SEO 是核心流量來源,不可犧牲
- **要決議的**:後端實作 `GET /api/products/{id}`,回傳 shape 同列表單筆(信封 `{success, data}`;404 時回業務錯誤 `{success:false, error:{code:'NOT_FOUND'}}`)。查詢邏輯與列表相同、只差 `WHERE id = ?`,預估半天內可完成
...
### #3 — `ingredients` 被拆陣列,違反 2026-05-06 拍板 【v4 降級:篩選已通,剩顯示層】
...
- **v4 複測更新(2026-08-24~26)— 情勢大幅好轉**:
  - ✅ **成分篩選已可用**:`?ingredient=雞肉` 回 856 筆、`?excludeIngredient=雞肉` 回 1231 筆,856+1231=2087 完美互補;比對方式為子字串模糊比對(`ingredient=肉` 回 1388、字典外的 `雞肉粉` 回 140)。前端 5+1 篩選 UI 經 live 站實測全通。v3 寫的「篩選無法運作」已不成立
  - ✅ **字典乾淨**:`/api/filters.ingredients` 為 13 條策展過的蛋白質來源(雞肉 856、鮭魚 460、鮪魚 274…),無斷裂垃圾條目
  - 🟡 **剩餘問題只在詳情頁顯示**:前端已用「、」黏回陣列當降級顯示,多數案例可讀;但拆陣列造成的損毀分兩級 —
    - 輕傷(可還原):id 1959 `["綜合礦物質(鋅","鐵","銅","錳","碘)"]` — 子配方語意消失,黏回後恰可復原
    - 重傷(**不可逆**):id 1003 `["增稠劑(改質性澱粉","增稠多醣)","礦物質(糖殼","磷","氯物質)胺基酸)"]` — 詞被熔接、括號錯亂,原文無法從陣列反推,黏回後仍有「胺基酸)」懸空斷片
- **要決議的(v4 修訂,訴求從「改回字串」改為「加法」)**:
  - 後端的陣列/正規化設計**保留不動**(篩選與計數繼續用它,運作良好);只需把爬蟲手上的原始字串**多加一個 `ingredientsText` 欄位**回 response(nutritionText 已是同樣做法),顯示問題即全解、id 1003 類不可逆損毀不再影響使用者
  - 2026-05-06 拍板同步更新:改記為「陣列與原始字串並存,各司其職」
...
1. **先修 #0**(間歇 500)— v4 三天連續重現、模式收斂為「首次請求易 500、重試即恢復」,請優先查連線池/冷啟動;順便加健康檢查
2. **實作 #13**(`GET /api/products/{id}`)— 上線前唯一的功能缺口(分享連結/SEO/重新整理),查詢同列表只差 `WHERE id = ?`
3. **#8 補拆纖維** + **#3 加回 `ingredientsText` 欄位** — 兩者資料都在後端手上,合計 1 小時等級;#3 已不擋上線但順手做掉最划算
4. **重議 #1**(濕食 6.5%:加來源站 vs 改 Phase 1 範圍)— 產品決策
5. **#4 variants[] 規格化** + **#2 age enum 拍板** — 文件/字典決策
6. **#5-#7、#9 文件補登/統計請求** — 不阻塞
7. **#10 品牌治理**(512 個品牌大量中英重複,live 示範站篩選器一眼可見)— 可 Phase 1 內處理也可推 Phase 2

> **給後端的整體訊息(v4)**:改 code 的硬需求只剩 #0/#13 兩項 + #8/#3 兩個順手項。前端功能全數完成、兩個示範環境已上線 — mock 版看「補完後的樣子」,live 版當場重現 500 與詳情頁 404。#0/#13 解掉後,前端切正式上線只是改一個環境變數。
...
| 2026-08-26 | v4:全端點複測(8/24~26)。**#3 大幅降級** — 實測 `ingredient`/`excludeIngredient` 篩選已可用(856+1231=2087 互補)、字典乾淨 13 條,剩顯示層,訴求改為「加回 `ingredientsText` 欄位」並附斷裂實例(id 1959 輕傷/id 1003 不可逆);#0 補三天重現記錄(模式:首次 500、重試即過);#13 補前端過渡方案說明(站內導覽已救、分享/SEO 仍死);新增「線上示範環境」(mock 版 + live 版 Vercel 網址);優先順序改版(硬需求縮至 #0/#13) |
